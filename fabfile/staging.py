...
STAGING = config.HOSTS['staging']['ip']
USER = config.HOSTS['staging']['user']
PASSWD = config.HOSTS['staging']['password']
HOST_PKGS = config.HOSTS['staging']['packages']
PY_PKGS = config.HOSTS['staging']['python_packages']
...
def bootstrap():
    ...
    if not exists("/etc/machine_deployed"):
        ...
        users.user_add(STAGING, "webapp", "system")
        users.user_add(STAGING, "andrew")
